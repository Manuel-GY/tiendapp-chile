...
async function processFlowPayment(token, env) {
  const FLOW_API_KEY = env.FLOW_API_KEY;
  const FLOW_SECRET_KEY = env.FLOW_SECRET_KEY;
  const RESEND_API_KEY = env.RESEND_API_KEY;
  const RESEND_FROM = env.RESEND_FROM || "ventas@tiendapp.cl";
  ...
  const statusResponse = await fetch(baseUrl + "/api/payment/getStatus?" + qs);
...
export async function onRequestPost(context) {
  try {
    ...
    if (token) {
      await processFlowPayment(token, env);
    }
...
function buildEmailHtml(payment) {
  const paymentId = payment.commerceOrder || payment.flowOrder;
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:w400;500;600;700;800&display=swap" rel="stylesheet">
  <style>
...
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="order-box">
        <div class="order-row"><span class="label">Producto</span><span class="value">Sistema Light Retro</span></div>
        <div class="order-row"><span class="label">Monto pagado</span><span class="value">$${amount ? amount.toLocaleString("es-CL") : "10.000"} CLP</span></div>
        <div class="order-row"><span class="label">ID de compra</span><span class="value">#${paymentId}</span></div>
        <div class="order-row"><span class="label">Fecha</span><span class="value">${date}</span></div>
      </div>
      <div class="divider" style="margin-bottom: 25px;"></div>
      <h2 class="section-title">Guia de Instalacion</h2>
      <div class="guide-section">
        <div class="guide-header"><h3>&#127918; Edicion Basica - PC (Sistema Light 50GB)</h3></div>
        <div class="guide-body">
          ...
          <div class="step"><div class="step-num">1</div><div class="step-text"><strong>Descarga el archivo ZIP</strong> y guardalo en un lugar facil de encontrar.</div></div>
          <div class="step"><div class="step-num">2</div><div class="step-text">Verifica que tengas al menos <span class="hl">120 GB libres</span> en tu disco.</div></div>
          <div class="step"><div class="step-num">3</div><div class="step-text">Clic derecho sobre el archivo &rarr; <span class="hl">"Extraer todo"</span>. Ruta recomendada: <strong>C:\\EmulaPlays</strong></div></div>
          <div class="step"><div class="step-num">4</div><div class="step-text">Abre la carpeta <strong>GPBOXPC</strong> y ejecuta el archivo <span class="hl">.exe</span> principal.</div></div>
          <div class="step"><div class="step-num">5</div><div class="step-text">Conecta tu control por USB o Bluetooth. Ve a <span class="hl">Configuracion &rarr; Controles</span> para mapear botones.</div></div>
        </div>
      </div>
      <div class="guide-section">
        <div class="guide-header" style="background: rgba(0,200,83,0.08);"><h3>&#127942; Edicion Definitiva - PC (Sistema Full 380GB)</h3></div>
        <div class="guide-body">
          ...
          <div class="step"><div class="step-num">1</div><div class="step-text"><strong>Descarga el archivo RAR</strong>. Es pesado (~71 GB), ten paciencia.</div></div>
          <div class="step"><div class="step-num">2</div><div class="step-text">Instala <span class="hl">7-Zip</span> (gratis en 7-zip.org) o <strong>WinRAR</strong> para descomprimir.</div></div>
          <div class="step"><div class="step-num">3</div><div class="step-text">Necesitas al menos <span class="hl">700 GB libres</span> para descomprimir.</div></div>
          <div class="step"><div class="step-num">4</div><div class="step-text">Clic derecho &rarr; <span class="hl">"Extraer aqui"</span>. Ruta: <strong>C:\\EmulaPlays\\GPBOXPC</strong></div></div>
          <div class="step"><div class="step-num">5</div><div class="step-text">La extraccion puede tardar <strong>30+ minutos</strong>. No cierres la ventana.</div></div>
          <div class="step"><div class="step-num">6</div><div class="step-text">Ejecuta el archivo <span class="hl">.exe</span> principal y configura tus controles.</div></div>
        </div>
      </div>
      <div class="tip-box"><strong>&#9888;&#65039; Importante:</strong> Si tu antivirus bloquea archivos, agrega la carpeta de EmulaPlays a las excepciones. Si tienes problemas, contactanos por WhatsApp.</div>
